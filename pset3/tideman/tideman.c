...
void add_pairs(void)
{
    int maxpair = candidate_count * (candidate_count - 1) / 2;
...
// Uses recursion to see if cycle is created
bool cycle(int loser, int winner)
{
    // Base case
    if (loser == winner)
    ...
    // Go through all candidates in locked array
    for (int i = 0; i < candidate_count; i++)
    {
        // If pair is locked already
        if (locked[loser][i])
        {
            // See if loser wins over someone who wins over winner
            if (cycle(i, winner))
            {
                return true;
            }
